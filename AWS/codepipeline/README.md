...
- `CODE_OF_CONDUCT.md`: This file outlines the expected code of conduct for contributors to this project.

- `CONTRIBUTING.md`: This document provides comprehensive guidelines on how to contribute effectively to this project.

- `LICENSE`: This file contains the licensing information for the project. Make sure to review it to understand the terms and conditions.

- `README.md`: The current file you are reading. It serves as the primary guide to the project, explaining its structure and purpose.

- `data.tf`: This Terraform configuration file defines data sources required for various aspects of the infrastructure.

- `examples/terraform.tfvars`: An example Terraform variables file that showcases how to set up variables used in the Terraform scripts.

- `locals.tf`: This file contains local values that can be referenced within your Terraform configuration to improve readability and maintainability.

- `main.tf`: The main Terraform configuration file that defines the core infrastructure components of the CI/CD pipeline.

- `modules/`: This directory holds reusable Terraform module definitions for different aspects of the infrastructure:

- `codebuild/`: This module sets up the AWS CodeBuild project, responsible for building the application code.

- `codecommit/`: This module creates an AWS CodeCommit repository to store your application's source code.

- `codepipeline/`: Here, you'll find the module that defines the AWS CodePipeline, orchestrating the CI/CD flow.

- `iam-role/`: This module sets up an IAM role with the necessary permissions for various components.

- `kms/`: The KMS module creates an AWS Key Management Service key used for encryption purposes.

- `s3/`: This module defines the AWS S3 bucket required to store artifacts and other relevant files.

- `templates/`: This directory contains template files used within the infrastructure:

- `buildspec_apply.yml`: An example build specification file for applying changes during the deployment phase.

- `buildspec_destroy.yml`: An example build specification file for destroying resources.

- `buildspec_plan.yml`: An example build specification file for planning changes before deployment.

- `buildspec_validate.yml`: An example build specification file for validating code or configurations.

- `scripts/`: Here, you'll find scripts used within the infrastructure:

- `tf_ssp_validation.sh`: A script for performing Terraform static security analysis.

- `variables.tf`: This file defines input variables used throughout the Terraform configuration to customize the infrastructure to your needs.
